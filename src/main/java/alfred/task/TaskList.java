...
public class TaskList {
     ...
     * @throws AlfredException If the task ID supplied is invalid.
     */
    public String markTaskAsDone(int taskID, Storage storage) throws AlfredException {
        if (taskID < 1 || taskID > tasks.size()) {
            throw new AlfredException("Task ID is invalid!");
        }
     ...
     * @throws AlfredException If the task ID supplied is invalid.
     */
    public String deleteTask(int taskID, Storage storage) throws AlfredException {
        if (taskID < 1 || taskID > tasks.size()) {
            throw new AlfredException("Task ID is invalid!");
        }
     ...
     * @throws AlfredException if the DateTime format is invalid.
     */
    public String taskListToDateFilteredString(String dateString) throws AlfredException {
        LocalDate date;
     ...
     * @throws AlfredException If the tasks in the text file are stored incorrectly or
     * if there are errors reading from the file.
     */
    public void loadDataFromStorage(Path filePath) throws AlfredException {
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (entry.length != 3) {
                    throw new AlfredException("One or more entries in storage have an invalid number of arguments");
                }
                ...
                if (!status.equals("TRUE") && !status.equals("FALSE")) {
                    throw new AlfredException("One or more task statuses are not stored correctly");
                }
                ...
                case "DEADLINE":
                    ...
                    break;
                default:
                    throw new AlfredException("One or more task types are not stored correctly");
                }
            ...
            throw new AlfredException("Could not read from storage");
        }
