...
public class Event extends Task {
     ...
     */
    public static Event createNewEvent(String argument) throws AlfredException {
        ...
        if (eventArguments.length != 2) {
            throw new AlfredException("Invalid arguments for a new event.");
        }

        String eventName = eventArguments[0];
        if (eventName.isBlank()) {
            throw new AlfredException("Event name cannot be blank!");
        }

        String startDateTime = eventArguments[1];
        if (startDateTime.isBlank()) {
            throw new AlfredException("Event time cannot be blank!");
        }
