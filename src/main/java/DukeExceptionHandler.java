public class DukeExceptionHandler {
    ...
    public static String checkForException_new(String userInput) throws DukeException {
        String[] replyArr = null;
        DukeException exception = null;
        if (userInput.length() > 0) {
            replyArr = userInput.split(" ");
        ...
        if (userInput.length() == 0) {
            exception = new EmptyTaskException();
            // shortest userInput allowed is "list"
            // also unexpected if it doesn't contain/equal to any of the keywords:
            // "todo, deadline, event, done, delete, list, find"
        } else if (userInput.length() < 4
                || (!userInput.contains("todo") && !userInput.contains("deadline")
                && !userInput.contains("event") && !userInput.contains("done")
        ...
        } else if (!replyArr[0].trim().equals("todo") && !replyArr[0].trim().equals("event")
                    && !replyArr[0].trim().equals("deadline") && !replyArr[0].trim().equals("list")
                    && !replyArr[0].trim().equals("done") && !replyArr[0].trim().equals("find")
                    && !replyArr[0].trim().equals("delete")) {
            exception = new UnexpectedInputException();
        }
        if (exception == null) {
            return "NO EXCEPTION";
