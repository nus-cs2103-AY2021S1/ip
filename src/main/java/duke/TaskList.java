...
public class TaskList {
    ...
    void handleList(MyString respond) {
        respond.addNewLines("Here are the tasks in your list:");
        // list with index
        for(int i = 1; i <= getSize(); ++i) {
            respond.addNewLines(i + "." + tasks.get(i - 1).getStatus());
        }
        // list without index
        // tasks.getArrayList().forEach((n) -> System.out.println(n.getStatus()));
    }

    private int stringToInt(String value) throws DukeException {
        int c = 0;
    ...
    private int parseDone(String cmd) throws DukeException {
        String value = cmd.substring(DONE.length()).trim();
    ...
    private String parseToDo(String cmd) throws DukeException {
        String getName = cmd.substring(TODO.length()).trim();
    ...
    private ArrayList<String> parseDeadline(String cmd) throws DukeException {
        ...
        for(int i = 0; i < cmd.length(); ++i) {
            if(cmd.startsWith(BY_TAG, i)) {
                getName = cmd.substring(DEADLINE.length(), i).trim();
                getDeadline = cmd.substring(i + BY_TAG.length()).trim();
                break;
            }
        }
    ...
    private ArrayList<String> parseEvent(String cmd) throws DukeException {
        ...
        String getTime = "";
        for(int i = 0; i < cmd.length(); ++i) {
            if(cmd.startsWith(AT_TAG, i)) {
                getName = cmd.substring(EVENT.length(), i);
                getTime = cmd.substring(i + AT_TAG.length());
                break;
            }
        }
    ...
    private int parseDelete(String cmd) throws DukeException {
        String value = cmd.substring(DELETE.length()).trim();
        return stringToInt(value);
    }
    ...
    private String parseFind(String cmd) throws DukeException {
        String pattern = cmd.substring(FIND.length()).trim();
    ...
    public void handleFind(String cmd, MyString response) {
        try {
            ...
            for(int i = 1; i <= getSize(); ++i) {
                if(tasks.get(i - 1).description.contains(pattern)) {
    ...
    private ArrayList<String> parseDoWithin(String cmd) throws DukeException {
        ...
        String from = "";
        String to = "";
        for (int i = DO_WITHIN.length(); i < cmd.length(); ++i) {
            if (cmd.startsWith(BETWEEN_TAG, i)) {
                getName = cmd.substring(DO_WITHIN.length(), i);
                for (int j = i + DO_WITHIN.length(); j < cmd.length(); ++j) {
                    if (!cmd.startsWith(AND_TAG, j)) {
                        continue;
                    }
                    from = cmd.substring(i + DO_WITHIN.length(), j);
                    to = cmd.substring(j + AND_TAG.length());
                    break;
                }
                break;
            }
